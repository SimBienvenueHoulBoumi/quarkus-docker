...
import org.acme.gateway.client.OrdersServiceClient;
...
public class OrdersGatewayResource {
    ...
    @PATCH
    @Path("/{id}/status")
    public Object updateOrderStatus(@Context HttpHeaders headers, @PathParam("id") Long id, Object request) {
        String authorization = headers.getHeaderString(HttpHeaders.AUTHORIZATION);
        return ordersServiceClient.updateOrderStatus(authorization, id, request);
    }
}
